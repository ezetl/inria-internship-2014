...
others (for instance, the binding threads to cores optimization showed
a real speedup in simulations using bigger amount of threads, such as
16 or 24, while using busy waiters proved to be better than futexes in
...
simulations (under 300000 nodes), but remained almost the same
