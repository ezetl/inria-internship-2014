...
Figure \ref{fig:six} shows that this new implementation proved to be
faster than the original parallel version with sizes under 300000
nodes, while with bigger amount of nodes the speedup remains almost
the same. However, this approach also avoids the increase of the
threshold to unrealistic values (which may happen in the naive
...
\State
\Comment {Average amount of processes parallel and sequential}
\State $avg\_par\_proc, avg\_seq\_proc$
\State
\Comment {Standard deviation of processes parallel and sequential}
...
(3.12). The changes proposed worked in some scenarios better than
others (for instance, binding of threads to cores showed a real
speedup in simulations using bigger amount of threads, such as 16 or
24, while using busy waiters proved to be better than futexes in
simulations with small sizes and small amount of threads). Also, some
of the modifications did not affect the overall performance, or even
made it worst, like the parmap changes proposed in Section
\ref{sec:parallel}.

Most of the changes proposed increased execution speed with small size
simulations (under 300000 nodes), but they did not improve the bigger
ones, where the speedup is already achieved with our model.
